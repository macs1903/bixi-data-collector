...
# Construire la liste complète des stations avec calcul saturation / vide
stations_list = []
for s in status:
    ...
    saturation = (num_bikes_total / capacity) * 100 if capacity else None
    vide = (s.get('num_docks_available', 0) / capacity) * 100 if capacity else None
    
    stations_list.append({
        "station_id": s['station_id'],
        "name": station_info.get('name', 'N/A'),
        "address": station_info.get('address', 'N/A'),
        "legacy_id": station_info.get('legacy_id', 'N/A'),
